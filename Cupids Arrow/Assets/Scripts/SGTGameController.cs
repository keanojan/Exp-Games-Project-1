...
namespace ShootingGallery
{
	...
	public class SGTGameController:MonoBehaviour 
	{
		...
		public static string Selection = "Love";
		...
		// The number of bullets left
		internal float ammoLeft2;

		// The image showing how many bullets we have left
		public Image ammoBar2;

		// The width of a single bullet in the ammo bar
		public float ammoBarWidth2 = 21;
		...
		public void UpdateAmmo2()
		{
			...
			{
				// Update the ammo left by scaling the width of the AmmoFull bar
				ammoBar2.rectTransform.sizeDelta = new Vector2( ammoLeft2 * ammoBarWidth2, ammoBar2.rectTransform.sizeDelta.y);

				// Set the AmmoEmpty width based on the number of bullets we have multiplied by the width of a single bullet in the bar
				ammoBar2.transform.Find("Empty").GetComponent<Image>().rectTransform.sizeDelta = new Vector2( ammo2 * ammoBarWidth2, ammoBar2.rectTransform.sizeDelta.y);
			}

			if (Selection == "Love")
				Selection = "Friendzone";
		...
		public void Shoot()
		{
			if ( startDelay <= 0 && ammoLeft > 0 && shotObject && Time.deltaTime > 0 && Selection == "Love")
			{
			...
			if ( startDelay <= 0 && ammoLeft2 > 0 && shotObject && Time.deltaTime > 0 && Selection == "Friendzone" )
			{
				// Create a new shot at the position of the mouse/tap
				Transform newShot = Instantiate( shotObject ) as Transform;
		...
		public void Reload2()
		{
			...
			if ( soundSource && soundReload )    
			{
				...
				soundSource.GetComponent<AudioSource>().PlayOneShot(soundReload2);
			}
		}
